 ...
 * @param partial Specified signal level is only partially achieved: the last
 *                dot is drawn hollow because amplitude was measured but is
 *                outside the physical range.
 * @param absent The last dot is not drawn at all because no amplitude could
 *               be measured.
 * @param happy Green happy face or red frowny face.
 * @param light Indicate light sampling mode.
 * @return Y coodinate of top margin.
 */
static double draw_watch_icon(cairo_t *c, int signal, bool partial, bool absent, bool happy, bool light)
{
...
static gboolean output_draw_event(GtkWidget *widget, cairo_t *c, struct output_panel *op)
{
	...
	double x = draw_watch_icon(c, snst->signal, snst->amp > 0 && !snst->amp_valid, snst->amp <= 0,
	...
	if(snst->calibrate) {
		...
		if(p) {
			...
			if(snst->amp > 0)
				sprintf(outputs[4], snst->amp_valid ? " %3.0f" : " %3.0f*",
				        snst->amp);
			else
				strcpy(outputs[4]," ---");
		} else {
...
static gboolean paperstrip_draw_event(GtkWidget *widget, cairo_t *c, struct output_panel *op)
{
	 ...
	 * only advancing time by a multiple of a row.  Round up so the newest
	 * event does not appear to be from the future.  */
	...
	cairo_set_font_size(c, 10);  // minute labels use a small fixed size font
	for(i = 0; ; i++) {
		...
		if (minute) {
			...
			cairo_set_source(c, white);
			// Draw the label horizontally, centered in the left margin
			cairo_text_extents(c, s, &extents);
			cairo_move_to(c, left_margin/2 - extents.x_advance/2, row - 2);
			cairo_show_text(c, s);
		}
	}
	...
	for (i = snst->events_count; i > 0; i--) {
		...
		debug("point %2d: %7llu, cycle %.1f, offset %.1f ms, chart phase = %.1f ms, column %.0f\n",  idx, (unsigned long long)event, cycles,
...
static void handle_zoom(GtkScaleButton *b, gdouble value, struct output_panel *op)
{
	...
	/* Attempt to position anchor_offset at same point in chart in new scale */
	if (ssd->beat_scale)
...
struct output_panel *init_output_panel(struct computer *comp, struct snapshot *snst, int border)
{
	...
	// Zoom slider and reset button for the paperstrip
	...
	gtk_box_pack_start(GTK_BOX(hbox3), op->zoom_button, FALSE, FALSE, 0);
	...
	gtk_box_pack_start(GTK_BOX(hbox3), op->zoom_orig_button, FALSE, FALSE, 0);
