...
static int scan_snapshot(FILE *f, struct snapshot **s, char **name)
{
	...
	*name = NULL;
	...
	for(;;) {
		...
		SCAN(double,be);
		SCAN(double,amp);
		if(!strcmp("amp_valid", l)) {
			debug("serializer: scanning amp_valid\n");
			if(scan_int(f, &((*s)->amp_valid))) goto error;
			amp_valid_found = 1;
			continue;
		}
	...
	// Old .tgj files lack the events_tictoc array; fall back to zero-fill
	if(!(*s)->events_tictoc)
